...
import torch
import pyrealsense2 as rs  # realsense摄像机自带的库
import math
import yaml
import argparse
import os
...
class YoloV5:
    ...
    """
        初始化
        self.yolov5        模型配置文件
        self.colors        随机生成每个类别的颜色
    """
    def __init__(self, yolov5_yaml_path='config/AppleYolov5s.yaml'):
        # 载入配置文件
    ...
    def init_model(self):
        ...
        """
            图像缓冲区初始化，创造一个全为零的张量img_torch
            self.yolov5['input_size']       输入图像的尺寸  input_size: 640
            self.yolov5['class_num']        类别个数  class_num: 2
        """
    ...
    """
        图像预处理
        img_resize          缩放后的图像
        img_arr             增加一个维度、转换为RGB、归一化之后的图像数组
    ...
    def detect(self, img, canvas=None, view_img=True):
        ...
        # ndimension() 函数返回张量的维度数如果 self.img_torch 是一个三维张量
        # 则使用 unsqueeze(0) 方法在张量的第一个位置增加一个维度
...
if __name__ == '__main__':
    ...
    model = YoloV5(yolov5_yaml_path='config/AppleYolov5s.yaml')
        ...
        while True:
            ...
            if xyxy_list:
                for i in range(len(xyxy_list)):
                    ...
                    print(camera_xyz)
                    # print(f"{camera_xyz[0]}," + f"{camera_xyz[1]}," + f"{camera_xyz[2]}")

                    # 打开文件，准备写入
                    with open('data.txt', 'w') as file:
                        # 将变量的值转换为字符串，然后写入文件
                        file.write(f"{camera_xyz[0]},{camera_xyz[1]},{camera_xyz[2]}")
                        # time.sleep(0.5)
